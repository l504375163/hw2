...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        list_of_discounted_returns = [r * self.gamma**t for t, r in enumerate(rewards)]
        list_of_discounted_returns = np.sum(list_of_discounted_returns).repeat(len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        list_of_discounted_returns = [r * self.gamma ** t for t, r in enumerate(rewards)]
        reverse_list = np.flip(list_of_discounted_returns)
        list_of_discounted_cumsums = np.flip(np.cumsum(reverse_list))
        discounts = np.array([self.gamma**(-t) for t in range(len(rewards))])
        list_of_discounted_cumsums *= discounts
